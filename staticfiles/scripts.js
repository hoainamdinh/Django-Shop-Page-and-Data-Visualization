// ------------------- DỮ LIỆU TỪ DJANGO -------------------
// Biến globalData sẽ được set từ template Django
// let globalData sẽ được override từ template
let formattedData = null;
let rawData = null;

// Hàm load dữ liệu từ Django template
function loadDataFromDjango() {
    try {
        console.log("🔍 Checking for data sources...");
        ...
        // Kiểm tra xem globalData đã được set từ template chưa
        if (typeof window.globalData !== 'undefined' && window.globalData && Array.isArray(window.globalData)) {
            rawData = window.globalData;
            console.log("✅ Dữ liệu từ window.globalData:", rawData.length, "records");
        } else if (typeof globalData !== 'undefined' && globalData && Array.isArray(globalData)) {
            rawData = globalData;
            console.log("✅ Dữ liệu từ biến globalData:", rawData.length, "records");
        } else {
            console.error("❌ Không tìm thấy dữ liệu từ Django template");
            console.log("   Available window properties:", Object.keys(window).filter(k => k.includes('Data') || k.includes('data')));
            
            // Tạo sample data để test visualization
            console.log("🔧 Sử dụng sample data để test...");
            rawData = [
                {
                    'thoi_gian_tao_don': '2022-01-01T08:01:09',
                    'ma_don_hang': 'ORD0000001',
                    'ma_khach_hang': 'CUZ00001',
                    'khach_hang': 'Lê Vũ Tâm',
                    'ma_pkkh': 'A1',
                    'mo_ta_pkkh': 'Nhân viên văn phòng',
                    'ma_nhom_hang': 'BOT',
                    'ten_nhom_hang': 'Bột',
                    'ma_mat_hang': 'BOT01',
                    'ten_mat_hang': 'Bột cần tây',
                    'don_gia': 40000,
                    'so_luong': 1,
                    'thanh_tien': 40000,
                    'Thời gian tạo đơn': '2022-01-01T08:01:09',
                    'Mã đơn hàng': 'ORD0000001',
                    'Mã mặt hàng': 'BOT01',
                    'Tên mặt hàng': 'Bột cần tây',
                    'Mã nhóm hàng': 'BOT',
                    'Tên nhóm hàng': 'Bột',
                    'SL': 1,
                    'Đơn giá': 40000,
                    'Thành tiền': 40000,
                    'Mã khách hàng': 'CUZ00001',
                    'Khách hàng': 'Lê Vũ Tâm'
                },
                {
                    'thoi_gian_tao_don': '2022-01-01T08:01:09',
                    'ma_don_hang': 'ORD0000001',
                    ...
                    'mo_ta_pkkh': 'Nhân viên văn phòng',
                    'ma_nhom_hang': 'SET',
                    'ten_nhom_hang': 'Set trà',
                    'ma_mat_hang': 'SET03',
                    'ten_mat_hang': 'Set 10 gói trà hoa cúc trắng',
                    'don_gia': 150000,
                    'so_luong': 1,
                    'thanh_tien': 150000,
                    'Thời gian tạo đơn': '2022-01-01T08:01:09',
                    'Mã đơn hàng': 'ORD0000001',
                    'Mã mặt hàng': 'SET03',
                    'Tên mặt hàng': 'Set 10 gói trà hoa cúc trắng',
                    'Mã nhóm hàng': 'SET',
                    'Tên nhóm hàng': 'Set trà',
                    'SL': 1,
                    'Đơn giá': 150000,
                    'Thành tiền': 150000,
                    'Mã khách hàng': 'CUZ00001',
...
// Mapping tên cột (giữ nguyên)
const columnMapping = {
    'thoi_gian_tao_don': 'Thời gian tạo đơn',
...
function initializeData(rawData) {
    if (Array.isArray(rawData)) {
        ...
        console.error("❌ Dữ liệu không đúng định dạng:", rawData);
        formattedData = [];
    }
...
function loadData() {
    return new Promise((resolve, reject) => {
        // Load dữ liệu từ Django nếu chưa có
        if (!rawData) {
        ...
        if (!formattedData || !Array.isArray(formattedData) || formattedData.length === 0) {
            reject(new Error("Dữ liệu chưa được thiết lập hoặc rỗng."));
            return;
        }

        const data = formattedData;

        // Vì dữ liệu từ Django đã được mapping sẵn trong views.py, 
        // chúng ta chỉ cần xử lý kiểu dữ liệu
        const processedData = data.map(item => {
            const newItem = { ...item }; // Copy toàn bộ item
            
            // Xử lý các trường số
            newItem['SL'] = +newItem['SL'] || +newItem['so_luong'] || 0;
            newItem['Đơn giá'] = +newItem['Đơn giá'] || +newItem['don_gia'] || 0;
            newItem['Thành tiền'] = +newItem['Thành tiền'] || +newItem['thanh_tien'] || 0;

            // Xử lý thời gian
            if (newItem['Thời gian tạo đơn']) {
                newItem['Thời gian tạo đơn'] = new Date(newItem['Thời gian tạo đơn']);
            } else if (newItem['thoi_gian_tao_don']) {
                newItem['Thời gian tạo đơn'] = new Date(newItem['thoi_gian_tao_don']);
            }

            // Đảm bảo các trường mapping được sao chép đúng
            newItem['Mã đơn hàng'] = newItem['Mã đơn hàng'] || newItem['ma_don_hang'];
            newItem['Mã mặt hàng'] = newItem['Mã mặt hàng'] || newItem['ma_mat_hang'];
            newItem['Tên mặt hàng'] = newItem['Tên mặt hàng'] || newItem['ten_mat_hang'];
            newItem['Mã nhóm hàng'] = newItem['Mã nhóm hàng'] || newItem['ma_nhom_hang'];
            newItem['Tên nhóm hàng'] = newItem['Tên nhóm hàng'] || newItem['ten_nhom_hang'];
            newItem['Mã khách hàng'] = newItem['Mã khách hàng'] || newItem['ma_khach_hang'];
            newItem['Khách hàng'] = newItem['Khách hàng'] || newItem['khach_hang'];
        ...
        console.log("📊 Dữ liệu sau khi xử lý:", globalData);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Đợi một chút để đảm bảo globalData từ template đã được set
    setTimeout(() => {
        loadData().then(() => {
            if (globalData && globalData.length) {
                ...
                d3.select("#title").html("❌ Không có dữ liệu hợp lệ.");
                console.log("❌ No valid data found");
            }
        }).catch(error => {
            console.error('❌ Error loading data:', error);
            d3.select("#title").html("❌ Lỗi khi tải dữ liệu: " + error.message);
        });
...
// --- TODO: Thêm các hàm q1()...q12() tại đây ---
...
function q1() {
    ...
    const margin = { top: 50, right: 300, bottom: 40, left: 300 }; // giảm top margin
    const width = 600; // tăng thêm cho legend
    const height = 600;
